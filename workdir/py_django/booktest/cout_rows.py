...
#代码所在目录
#FILE_PATH='./'
FILE_PATH='E:\\Python\\workdir\\py_django\\booktest'
def analyze_code(codefilesource):
    '''
    打开一个py文件，统计其中的代码行数，包括空行和注释
    返回该文件总行数，注释行数，空行数的列表
...
def run(FILE_PATH):
    ...
    for i in os.listdir(os.getcwd()):
        if os.path.split(i)[1]=='.py':
            line=analyze_code(i)
            total_lines,total_comment_lines,total_blank_lines=total_lines+line[0],total_comment_lines+line[1],total_blank_lines+line[2]
    print "总代吗行数：", total_lines
    print "总注释行数：",total_comment_lines,"占%0.2f%%"%(total_comment_lines*100.0/total_comment_lines)
    print "总空行数：",total_comment_lines,"占%0.2f%%" % (total_blank_lines*100.0/total_lines)
